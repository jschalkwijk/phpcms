...
try {
    ...
    // also try BLOBS.
    $query = "INSERT INTO crypto(text,token) VALUES('$storeMe','$key')";
...
// save the encryoted text as type VARBINARY in database. uitzoeken iets over CAST mysql functie.
try {
	 $dbc = mysqli_connect('localhost','root','root','nerdcms_db');
	 $query = "SELECT * FROM crypto WHERE id = 66";
