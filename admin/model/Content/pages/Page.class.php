...
class content_Pages_Page extends content_Content{
	...
	public function addPage($back,$front,$sub_page,$id = null){
		...
		$page_title = mysqli_real_escape_string($dbc->connect(),trim($this->title));
		$page_desc = mysqli_real_escape_string($dbc->connect(),trim($this->description));
		$page_content = mysqli_real_escape_string($dbc->connect(),trim($this->content));
		$author = $this->author;
		$tpl_URL = mysqli_real_escape_string($dbc->connect(),trim($_POST['template']));
		...
		if (!empty($page_title) && !empty($page_content) && (!empty($front) || !empty($back) )) {
			...
			$query = "INSERT INTO pages(title,description,content,author,date) VALUES('$page_title','$page_desc','$page_content','$author',NOW())";
			...
			$query2 = "SELECT page_id FROM pages WHERE title = '$page_title'";
			$page = mysqli_query($dbc->connect(),$query2);
			...
			while ($row = mysqli_fetch_array($page)) {
				...
				$query3 = "UPDATE pages SET path='$path',parent_id=$parent_id WHERE pages.page_id = '$pageID'";
				echo $query3;
				mysqli_query($dbc->connect(),$query3);
	...
	public static function create_path($page_id,$page_title){
		$dbc = new DBC;
		$query = "SELECT title,page_id,path FROM pages WHERE page_id = $page_id";
		$data = mysqli_query($dbc->connect(),$query) or die('Error creating page path');
	...
	public static function deletePage($multiple){
		$dbc = new DBC;
		$query = "SELECT p1.*,p2.title as pname FROM pages p1 INNER JOIN pages p2 ON p1.parent_id = p2.page_id WHERE p1.page_id IN({$multiple})";
		$data = mysqli_query($dbc->connect(),$query) or die('error connecting');

		if(mysqli_num_rows($data) == 0){
			// if num rows is zero, do normal query to the page to delete the title. because otherwise we wont get it.
			$query = "SELECT * FROM pages WHERE page_id IN({$multiple})";
			$data = mysqli_query($dbc->connect(),$query) or die('error connecting');
		}
		...
		while ($row = mysqli_fetch_array($data)) {
			if ($row['parent_id'] != 0 && $row['parent_id'] != $row['page_id']) {
				...
				if (unlink('./././controller/'.$row['title'].'.php')) {
					if ($row['parent_id'] == 0) {
						$page_id = $row['page_id'];
						$query = "DELETE FROM pages WHERE parent_id = $page_id";
						echo $query;
